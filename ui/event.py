...
from .style import suggestionBoxStyleSheet
...
class Window(Ui_MainWindow, QtWidgets.QMainWindow):
    ...
    def initLayout(self):
        ...
        if self.prj.model is None:
            ...
            self.importHint['analysisArea'].setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
    ...
    def action_write(self):
        try:
            ...
            filePath = QtWidgets.QFileDialog.getExistingDirectory(self, "Select the saving path", self.showDir)
            self.prj.model.write(geditor, filePath)
    ...
    def action_Simulation(self):
        try:
            if self.prj.model:
                ...
                filePath = QtWidgets.QFileDialog.getExistingDirectory(self, "Select the project path", self.showDir)
    ...
    def simulation(self):
        ...
        if filePath:
            ...
            self.prj.model.simulation(epw=filePath, overwrite=False, stdout=self.stdout, process_count=8)
    ...
    def action_Result_Folder(self):
        try:
            if not self.prj.model.folder:
                filePath= QtWidgets.QFileDialog.getExistingDirectory(self, "Select the result folder", self.showDir)
                if filePath:
    ...
    def action_SimParams(self):
        try:
            geditor = self.groupEditorArea.crossAllGroupEditors()
            filePath, filetype = QtWidgets.QFileDialog.getSaveFileName(self, "Select the saving path", self.showDir,
                                                                       'Csv Files (*.csv)')
            if filePath:
    ...
    def action_saveAs(self):
        try:
            ...
            if filePath:
                self.showDir = os.path.dirname(filePath)
    ...
    def action_load(self):
        try:
            filePath, filetype = QtWidgets.QFileDialog.getOpenFileName(self, "Select the project file", self.showDir,
                                                                       'Epeditor Zip Files (*.zip)')
            if filePath:
                self.showDir = os.path.dirname(filePath)
    ...
    def action_import_idf(self):
        try:
            filePath, filetype = QtWidgets.QFileDialog.getOpenFileName(self, "Select the baseline *.idf file", self.showDir,
    ...
    def cleanBrowser(self,reconnect=True):
        ...
        print('>>>How are you?')
        print("MoosasQA:I'm just a computer program, so I don't have feelings, but I'm here to help you! How can I assist you today?")
