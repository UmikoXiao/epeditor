...
class searchWidget(redirectQWidget):
    def __init__(self, parent, window, prj):
        ...
        self.gridLayout.addWidget(self.addbutton, 2, 0, 1, 6)
    ...
    def _create_SearchResultLayout(self, result: list):
        ...
        if len(result) == 1:
            self.searchResultWidget.setGeometry(QtCore.QRect(10, 10, itemWidth, itemHeight))
        elif len(result) == 2:
        ...
        for i in range(len(result)):
            ...
            self.SearchResultLayout.addWidget(
                IDFSearchResultBox(parent=self.searchResultWidget, item=result[i], rect=rect), np.floor(i / columns),
                i % columns)
    ...
    def addBox(self, row, col):
        try:
            if self.prj.library['DragEventObject']:
                ...
                searchbox = IDFSearchResultBox(parent=self, item=searchresult, rect=rect)
            ...
            try:
                ...
                if isinstance(gridnode, IDFSearchResultBox):
                    ...
                    if gridnode.showed:
                        ...
                        rect = QtCore.QRect(0, 0, self.itemwidth, self.itemheight)
                        searchresultbox = [IDFSearchResultBox(parent=self, item=searchresult, rect=rect) for
...
class groupEditorWidget(redirectQWidget):
    ...
    def editorFromCSV(self):
        try:
            ...
            if filePath:
                item = IDFGroupEditor.load(self.prj.model,filePath)
