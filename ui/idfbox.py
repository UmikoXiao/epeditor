...
class IDFSearchResultBox(QtWidgets.QCheckBox):
    def __init__(self, parent=None, item: IDFsearchresult = None, rect=None):
        ...
        try:
            ...
            if self.item:
                ...
                for ki, key in enumerate(self.idfobjectlist.keys()):
                    ...
                    # objTab.setMinimumHeight(rowHight)
...
class ResultBox(QtWidgets.QWidget):
    ...
    def initResultWidget(self):
        try:
            ...
            for i, method in enumerate([np.array,np.mean,np.median,np.std,np.min,np.max]):
                methodName = ['rawData','mean','median','std','min','max']
            ...
            self.gridLayout.addWidget(self.variables_Scroll.Title, 0, 1, 1, 2)
            self.gridLayout.addWidget(self.calculator.Title, 0, 3, 1, 1)
            self.gridLayout.addWidget(self.timeStep, 1, 0, 2, 1)
            self.gridLayout.addWidget(self.variables_Scroll, 1, 1, 1, 2)
            label_ = QtWidgets.QLabel('KeyWords:')
            label_.setStyleSheet(titlestylesheet)
            self.gridLayout.addWidget(label_, 2, 1, 1, 1)
            self.gridLayout.addWidget(self.variables_Scroll.variableSearch, 2, 2, 1, 1)
            self.gridLayout.addWidget(self.calculator, 1, 3, 2, 1)
            self.gridLayout.addWidget(self.exportResult, 3, 0, 1, 1)
            self.gridLayout.addWidget(self.visualize, 3, 1, 1, 3)
        ...
        try:
            ...
            try:
                result = self.prj.model.group_result(variable=_variables,
                                            cases=list(range(len(self.prj.model.sql))),
